...
func (repos *TodoListPostgres) GetAll(userId int) ([]todo.TodoList, error) {
	var lists []todo.TodoList

	query := fmt.Sprintf(`SELECT tl.id, tl.title, tl.description FROM %s tl
		INNER JOIN %s ul on tl.id = ul.list_id WHERE ul.user_id = $1`, todoListsTable, usersListsTable)

	err := repos.db.Select(&lists, query, userId)

	return lists, err

}

func (repos *TodoListPostgres) GetById(userId, listId int) (todo.TodoList, error) {

	var list todo.TodoList

	query := fmt.Sprintf(`SELECT tl.id, tl.title, tl.description FROM %s tl
	INNER JOIN %s ul on tl.id = ul.list_id WHERE ul.user_id = $1 AND tl.id = $2`, todoListsTable, usersListsTable)

	err := repos.db.Get(&list, query, userId, listId)

	return list, err
}

func (repos *TodoListPostgres) Delete(userId, listId int) error {
	...
	_, err := repos.db.Exec(query, userId, listId)

	return err
}

func (repos *TodoListPostgres) Update(userId, listId int, input todo.UpdateListInput) error {
